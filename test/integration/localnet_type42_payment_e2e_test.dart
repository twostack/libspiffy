...
///
/// Carol publishes her wallet's anchor key A, signed for her identity, and
/// goes offline. Alice derives a destination C from A with a fresh payer
/// key and an invoice number, pays it, broadcasts the payment herself (it
/// is hers to broadcast: Carol is not there to take it), and follows it to
/// its block. She exports it with its proof and the type-42 hand-off. Carol
/// comes online, derives C from her own anchor key, imports the payment,
/// and spends it on the network.
///
/// When Carol is online she takes the payment in unproven, with
/// ValidateBEEFCommand and the hand-off, as any payment handed to her.
///
...
void main() {
  ...
  Future<String> publishAnchor(LocalnetNode carol) async {
    ...
    carol.coordinator.tell(GetAnchorPublicKeyQuery(walletId: carolWallet));
    ...
    carol.coordinator.tell(SignWithAnchorKeyCommand(walletId: carolWallet, message: message));
  ...
  Future<Type42Destination> derive(String anchor) async {
    final requestId = 'derive-${requests++}';
    final derived = alice.next<Type42DestinationEvent>((e) => e.requestId == requestId);
    alice.coordinator.tell(
        DeriveType42DestinationCommand(walletId: aliceWallet, recipientPublicKey: anchor, requestId: requestId));
  ...
  test('Alice pays offline Carol at a type-42 destination of her anchor key, broadcasts it herself, and hands '
    ...
    // Alice derives the destination, pays it, and broadcasts the payment.
    ...
    // Carol comes online and is handed it.
