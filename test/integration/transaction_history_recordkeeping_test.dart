...
void main() {
  group('Transaction History Record-Keeping Integration Tests', () {
    ...
    test('Wallet records payment transaction in history after successful creation', () async {
      ...
      // Create the UTXO (the wallet aggregate handles the commands in order)
      libspiffy.walletManager.tell(
        WalletCommandMessage(
          testWalletId,
          ReceiveUTXOCommand(
            ...
            blockHeight: testBlockHeight,
            confirmations: 6,
            // The funding transaction's proof is imported above, so the
            // output is spendable; the command defaults to pending and
            // payments spend only available UTXOs.
      ...
      await _eventually(
        () async => (await storage.getPaymentUTXOs(testWalletId)).firstOrNull,
        'the funding UTXO to be available',
      );
      print('✓ Wallet funded');
      ...
      expect(paymentResponse.success, isTrue, reason: 'Payment creation should succeed: ${paymentResponse.error}');
      ...
      // Step 6: The payment coordinator replies once the projection has
      // applied the recording, so the history already holds it.
      ...
      // Step 8: Verify UTXOs are updated (should have change UTXO; the
      // output scan's event is projected after the recording)
      await _eventually(
        () async => (await storage.getUTXOs(testWalletId)).where((u) => u.txid == paymentTxid).firstOrNull,
        'the payment\'s change output to be a wallet UTXO',
      );
    ...
    test('Recipient wallet records incoming transaction after SPV validation', () async {
      ...
      // The key the fixture transaction pays (output 1 to its m/0/0 address).
      const recipientXpriv = 'tprv8ZgxMBicQKsPeMiDjtXBGAyFY1wEMGgomjwf54ZmiZfKTNYvVdBa6GqWUwnvtHm6NKVkQkhCKxaobd9JPxNEXgDfVgJ5RNHJ3ivogSG3V1R';
      ...
      // Step 2: setUp stored the real header of block 1239645.

      // Step 3: The address the fixture transaction pays (output 1) is the
      // recipient wallet's root address (m/0/0).
      final recipientAddress = 'mqCnSf8i6kmaQaJ54HjQ8EUJnuK4AnCv12';
      expect(recipientResponse.rootAddress, recipientAddress,
          reason: 'the fixture transaction must pay the recipient wallet');
      print('✓ Recipient address: $recipientAddress');

      // Step 4: The counterparty hands over the transaction with its merkle
      // proof (a BEEF); SPVActor validates it against the stored header and
      // records the wallet's output and the transaction.
      print('\n✓ Receiving payment via SPV...');
      final beef = BEEF.create(
        bumps: [fixtureBump()],
        txs: [Uint8List.fromList(hex.decode(_getTestTransactionHex()))],
        hasMerkle: [true],
        bumpIndex: [0],
      );
      final spvCompleter = Completer<SPVValidationResult>();
      final spvReceiver = await actorSystem.spawn(
        'recipient-spv-receiver',
        () => _TestReceiverActor<SPVValidationResult>(spvCompleter),
      );
      ...
      expect(spvResult.isValid, isTrue, reason: 'SPV validation should succeed: ${spvResult.validationError}');
      print('  ✓ SPV validation succeeded');

      // Step 5: Wait for the projection to record the output and the transaction
      await _eventually(
        () async => (await storage.getUTXOs(recipientWalletId)).firstOrNull,
        'the received UTXO in the read model',
      );
      await _eventually(
        () async => (await storage.getTransactionHistory(recipientWalletId)).firstOrNull,
        'the received transaction in the history',
      );
    ...
    test('Complete SPV payment flow using public APIs validates transaction and UTXO recording', () async {
      ...
      // The ARC service both parties use (no network).
      ...
      await _eventually(
        () async => (await senderStorage.getAvailableUTXOs(senderWalletId)).firstOrNull,
        'the sender\'s funding UTXO to be available',
      );
      await _eventually(
        () => senderStorage.getMerkleProof(testTxid),
        'the funding transaction\'s merkle proof',
      );
      ...
      final paymentResponse = await payInvoiceCompleter.future.timeout(Duration(seconds: 10));
      expect(paymentResponse.success, isTrue, reason: paymentResponse.error);
      ...
      await _eventually(
        () async => (await receiverStorage.getUTXOs(receiverWalletId)).where((u) => u.txid == paymentTxid).firstOrNull,
        'the receiver\'s payment UTXO in the read model',
      );
      await _eventually(
        () => receiverStorage.getTransaction(paymentTxid, walletId: receiverWalletId),
        'the receiver\'s payment transaction in the read model',
      );
      ...
      // SPV proves the payment's inputs exist (its ancestor's proof), not
      // that the payment is mined: without a proof of its own it stays
      // pending until the network has it (spv-understanding.md).
      expect(receiverTx.status, equals(TransactionStatus.pending), reason: 'Receiver tx should be PENDING until mined');
      ...
      final receiverUtxos = await receiverStorage.getUTXOs(receiverWalletId);
      ...
      expect(receivedUtxo.status, equals(UTXOStatus.pending),
          reason: 'An unmined payment is not spendable until the network has it');

      // ============================================================
      // Step 7: Receiver settles the payment through ARC (PUBLIC API)
      // ============================================================
      print('\n═══ Step 7: Receiver broadcasts; both wallets learn it from ARC ═══');
      final broadcastCompleter = Completer<dynamic>();
      final broadcastReceiver = await receiverActorSystem.spawn(
        'broadcast-receiver',
        () => _TestReceiverActor<dynamic>(broadcastCompleter),
      );
      receiverLibspiffy.arcActor.tell(
        BroadcastTransactionMessage(receiverWalletId, hex.encode(beef.txs.last), paymentTxid),
        sender: broadcastReceiver,
      );
      expect(await broadcastCompleter.future.timeout(Duration(seconds: 5)), isA<BroadcastSuccessMessage>());

      // ARC answered SEEN_ON_NETWORK: the receiver's output is spendable.
      final spendable = await _eventually(
        () async => (await receiverStorage.getAvailableUTXOs(receiverWalletId))
            .where((u) => u.txid == paymentTxid)
            .firstOrNull,
        'the receiver\'s payment UTXO to become available',
      );
      expect(spendable.satoshis, equals(invoiceAmount));
      print('  ✓ Receiver UTXO available once the payment is on the network');

      // The sender learns it through its status scan: its input is spent.
      senderLibspiffy.arcActor.tell(CheckStoragePendingUTXOsMessage(triggerBlockHeight: testBlockHeight));
      await _eventually(
        () async => (await senderStorage.getUTXOs(senderWalletId, includeSpent: true))
            .where((u) => u.key == '$testTxid:1' && u.status == UTXOStatus.spent)
            .firstOrNull,
        'the sender\'s funding UTXO to be spent',
      );
      print('  ✓ Sender funding UTXO spent once the payment is on the network');
      ...
      print('✅ Receiver:');
      print('   - Transaction recorded: ✓');
      print('   - Status: PENDING until mined ✓');
      print('   - Net amount positive: ✓');
      print('   - UTXO pending, available once on the network: ✓');
      print('✅ ═══════════════════════════════════════════════\n');
...
Future<void> _setupRealBlockHeaders(IsarWalletStorage storage) async {
  // The real header of testnet block 1239645, which contains the funding
  // transaction: the funding proof walks to its merkle root.
...
/// The funding transaction's real WhatsOnChain TSC proof (display-order
/// nodes), which verifies against [fixtureHeader].
