...
void main() {
  ...
  // BEEF carrying testnet tx a05924fc... (block 1239645, position 2) with a
  // BRC-74 BUMP built from its WhatsOnChain TSC proof (see conversion_test),
  // plus an unproven child. Regenerated for audit SPV-06: the previous
  // fixture's BUMP was emitted by the library's old non-standard layout.
  ...
  // Merkle root of testnet block 1239645 (display format)
  const String beefHEX2MerkleRoot = '4823b3e0a9801d019c49af6ecd923f5250cc828e7be4fb6b4c5afbb979e33b34';
  ...
  group('BEEF Tests', () {
    test('Parse this test BEEF and compute its merkle root', () {
      ...
      expect(beef.bumps[0].blockHeight, 1239645);

      // getVerifiedTransactions reports display-format txids
      final txid = txns.single['txid'] as Uint8List;
      expect(hex.encode(txid), 'a05924fcc63712d3e4b94b0c88baad234c2c8ad3d369704f53765e21a53a2101');
      expect(beef.bumps[0].computeMerkleRootForBlockHeader(txid), beefHEX2MerkleRoot);
      expect(beef.validateTransaction(txid), true);
      expect(hex.encode(beef.serialize()), beefHEX2);
    });
